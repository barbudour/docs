...
При генерации медиа контента SpeShu.AI автоматически:
...
Для постоянного хранения используйте Storage API с политикой `PERMANENT`.
...
Используйте GET /v1/media/{id} для проверки статуса и получения результата.