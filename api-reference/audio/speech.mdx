...
```bash
curl -X POST "https://speshu.ai/api/v1/audio/speech" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "openai/tts-1",
    "input": "Привет! Это SpeShu.AI!",
    "voice": "alloy"
  }'
```
...
response = requests.post(
    'https://speshu.ai/api/v1/audio/speech',
...
```javascript
const response = await fetch('https://speshu.ai/api/v1/audio/speech', {
  method: 'POST',
  headers: {
...
```json
{
  "audio": "https://cdn.speshu.ai/audio/9a0127a1.mp3",
