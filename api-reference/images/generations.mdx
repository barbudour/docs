...
Если вы разрабатываете новый софт — рекомендуем использовать Media API, который предоставляет единый интерфейс для всех медиа-операций.

Этот эндпоинт доступен по пути `/v2/images/generations`. При использовании OpenAI SDK с `base_url="https://speshu.ai/api/v1"` запросы автоматически направляются на правильный путь.
...
```bash
curl -X POST "https://speshu.ai/api/v2/images/generations" \
