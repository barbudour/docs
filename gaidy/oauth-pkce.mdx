...
OAuth PKCE позволяет сторонним приложениям получать API-ключи от имени пользователей SpeShu.AI — без ручного создания ключей. Пользователь авторизует приложение через consent screen, выбирает организацию, scopes и лимиты расходов, после чего приложение получает готовый API-ключ.

<Note>
  Если вы просто хотите использовать API напрямую — создайте ключ в [личном кабинете](https://speshu.ai/profile). OAuth PKCE нужен разработчикам сторонних приложений, которые запрашивают доступ у пользователей.
</Note>
